  ...
  return (
    <div className="h-screen grid-bg flex overflow-hidden">
          ...
          <>
            ...
            <div className="px-3 py-2 space-y-0.5">
              ...
              <button
                ...
                className={`cmd-btn w-full flex items-center gap-2.5 px-3 py-2 text-sm group ${
                  showBilling ? "text-orange-400 bg-orange-500/8" : "text-zinc-400 hover:text-orange-400"
                }`}
              >
      ...
      <div className="flex-1 flex min-w-0">
        ...
        {/* Billing Panel */}
        {showBilling && token && (
          <div className="w-96 border-l border-zinc-800/80 flex flex-col shrink-0 slide-in-right" style={{ background: "rgba(14,14,20,0.6)" }}>
            <div className="px-5 py-3 border-b border-zinc-800/80 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2">
                <CreditCard className="w-4 h-4 text-zinc-600" />
                <span className="text-sm font-medium text-zinc-400">Plans & Billing</span>
              </div>
              <button onClick={() => setShowBilling(false)} className="text-zinc-600 hover:text-zinc-400 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              <div className="glass-card p-4" style={{ borderRadius: "12px" }}>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Today's Usage</span>
                  <span className="text-xs text-zinc-600">
                    {usage.commands_today} / {usage.commands_limit === -1 ? "Unlimited" : usage.commands_limit}
                  </span>
                </div>
                <div className="w-full bg-zinc-800/50 rounded-full h-2">
                  <div
                    className="h-2 rounded-full transition-all bg-gradient-to-r from-orange-500 to-orange-400"
                    style={{ width: `${usage.commands_limit === -1 ? 5 : Math.min(100, (usage.commands_today / usage.commands_limit) * 100)}%` }}
                  />
                </div>
              </div>
                ...
                return (
                  <div
                    ...
                    className={`glass-card p-4 transition-all ${
                      isCurrent ? "border-orange-500/30 bg-orange-500/5" : "hover:border-zinc-700"
                    }`}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-semibold text-zinc-200">{plan.name}</span>
                        {isCurrent && (
                          <span className="status-badge bg-orange-500/15 text-orange-400 text-xs">Current</span>
                        )}
                      </div>
                      <div className="text-right">
                        {plan.price_monthly === 0 ? (
                          <span className="text-sm font-bold text-zinc-400">Free</span>
                        ) : (
                          <div>
                            <span className="text-sm font-bold text-zinc-200">${plan.price_monthly}</span>
                    ...
                    {isUpgrade && (
                      <button
                        ...
                        className="mt-3 w-full pro-btn px-3 py-2 rounded-lg flex items-center justify-center gap-1.5 text-xs font-medium"
                      >
